package com.example.lz.text.FileManager.Tool;
...
public class FileManager {
    ...
    static {
        ...
        if (Tools.getPhoneInSDCardPath() != null) {
            inSdcardDir = null;
            inSdcardDir = new File(Tools.getPhoneInSDCardPath());
        }
        // 如果有手机外置sdcard卡路径,进行外置Fiel实例化(防止File file = new File(null))
        if (Tools.getPhoneOutSDCardPath() != null) {
            outSdcardDir = null;
            outSdcardDir = new File(Tools.getPhoneOutSDCardPath());
        }
    }
    ...
    private void searchFile(File file, boolean isFirstRunFlag) {
        ...
        if (!file.isDirectory()) {
            ...
            //获取图标以及文件类型
            String[] iconAndTypeNames = Filetypechange
